...
var db = {
    ...
    editaAlumno : function (alumno,claveOri) {
      var index;
      //Buscamos el indice del alumno
      for (index = 0; index < this.alumnos.length; index++) {
        if(this.alumnos[index].clave == claveOri ){
          this.alumnos[index].clave = alumno.clave;
          this.alumnos[index].nombre = alumno.nombre;
          db.saveAlumnos();
        }
      }
    },
...
app.route("/alumnos")
  ...
  .put((req,res)=> {
    ...
    var aux = req.body;
    var alumno ={};
    alumno.nombre=aux.nombre;
    alumno.clave=aux.clave;
    var claveOri = aux.clave2;
    console.log("Objeto post recibido");
    console.log(alumno);
    console.log(claveOri);
    db.editaAlumno(alumno,claveOri);
    console.log(db.alumnos);
    res.json({'status' : 'OK'})
  });
